...
function already(sc){return !!sc&&(playedIds.has(String(sc.id||''))||playedSignatures.has(signature(sc)))}
...
function syntheticScene(sc){
  ...
  const host=$('.character-room')||$('#app')||document.body,b=document.createElement('button');b.type='button';b.hidden=true;b.dataset.scene=String(sc.id);b.dataset.hvContinuityBridge='7';host.appendChild(b);
  starting=true;bridge=true;document.body.classList.add(CHAIN);remember(sc);
  try{b.click()}finally{bridge=false;if(b.isConnected)b.remove();setTimeout(()=>{starting=false},80)}
  return true;
}
...
function finishAndContinue(button){
  ...
  const started=performance.now();let finishAttempts=0;
  const commitEnd=()=>{
    if(!button?.isConnected)return false;
    finishAttempts++;bridge=true;try{button.click()}finally{bridge=false}return true;
  };
  setTimeout(()=>{
    commitEnd();
    const wait=()=>{
      ...
      if($('.character-room .dialogue-box')){
        ...
        if(elapsed>140&&finishAttempts<2)commitEnd();
        if(elapsed>1500){continuing=false;stopChain(false);return}
        setTimeout(wait,24);return;
      }
...
function normalizeEnds(){
  if(!document.body.classList.contains(CHAIN))return;
  for(const b of document.querySelectorAll('.character-room .dialogue-box [data-end]')){
    b.dataset.hvContinuityEnd='7';
    if(String(b.textContent||'').replace(/\s+/g,' ').trim().toUpperCase().includes('RETURN'))b.innerHTML='NEXT <span>›</span>';
  }
}
...
document.addEventListener('click',e=>{
  ...
  if(scene){
    const id=String(scene.dataset.scene||''),r=roleForId(id),sc=sceneById(id);
    if(r==='CONVERSATION'){
  ...
  const talk=t.closest('.character-room [data-action="TALK"]');
  if(talk){e.preventDefault();e.stopImmediatePropagation();if(!startConversation({fresh:true}))stopChain();return}
  const end=t.closest('.character-room .dialogue-box [data-end]');
  if(end&&document.body.classList.contains(CHAIN)){e.preventDefault();e.stopImmediatePropagation();finishAndContinue(end)}
},true);
