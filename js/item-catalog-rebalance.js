(()=>{
'use strict';
if(window.__HELLAVERSE_ITEM_CATALOG_REBALANCE_V2__)return;
window.__HELLAVERSE_ITEM_CATALOG_REBALANCE_V2__=1;
...
function write(s){const v=JSON.stringify(s);localStorage.setItem(K,v);try{window.dispatchEvent(new StorageEvent('storage',{key:K,newValue:v}))}catch{}window.dispatchEvent(new CustomEvent('hellaverse:state-updated',{detail:{source:'item-catalog-rebalance-v2',clearDirty:false}}))}
const OWNER=[
...
function run(){
 ...
 if(changed){s.migrations=s.migrations||{};s.migrations.itemCatalogRebalanceV2={at:new Date().toISOString()};write(s)}
}
run();
})();