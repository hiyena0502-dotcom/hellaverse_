...
function looksNarration(v){
  const t=String(v||'').replace(/\s+/g,' ').trim();if(!t||/^[“"「『]/.test(t))return false;
  ...
  const stageEnding=/다\.$/;
  if(/^(?:그는|그가|그녀는|그녀가|당신은|당신이|상대는|상대가)\s/.test(t)&&actionCue.test(t)&&stageEnding.test(t))return true;
  if(/^(?:방 안|침대 위|책상|벽에|문 쪽|작업대|복도|피아노|창가|테이블|바닥|서랍|액자|의자|왕좌|엘리베이터|호텔)\S*\s/.test(t)&&stageEnding.test(t))return true;
  const subject=t.match(/^([가-힣A-Za-z·.' -]{2,24})(은|는|이|가)\s/);
  return !!(subject&&!/^(?:나|내|너|네|우리|그것|이것|저것|사람들?)$/.test(subject[1].trim())&&actionCue.test(t)&&stageEnding.test(t));
}
