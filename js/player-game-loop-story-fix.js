(()=>{
if(window.__HELLAVERSE_PLAYER_GAME_LOOP_STORY_FIX_V1__)return;
window.__HELLAVERSE_PLAYER_GAME_LOOP_STORY_FIX_V1__=1;
...
function showHotelMeeting(state){
 ...
 hotelMeetOpen=true;
 const origin=originKey(state),root=document.createElement('div');root.id='hvHotelMeetModal';root.className='hv-story-modal';
...
function enhanceLetterModal(state){const root=$('#hvGameModal');if(!root||root.dataset.storyEnhanced==='letter'||state.game?.currentEventId!=='hotel-errand'||state.game?.currentEventResolved)return;const kicker=$('.hv-game-kicker',root)?.textContent||'';if(!/TODAY'S EVENT/i.test(kicker))return;letterIntro(root,state)}
...
function decorateDayGuide(state){if(!state.player?.profileSetup||!state.game)return;const targets=$$('.hv-today-chip');for(const chip of targets){const old=chip.parentElement?.querySelector(':scope > .hv-day-guide');if(old)old.remove();chip.insertAdjacentHTML('afterend',dayGuideMarkup(state))}const modal=$('#hvGameModal .hv-game-modal-card');if(modal&&/DAY COMPLETE/i.test(modal.textContent||'')&&!$('.hv-day-explain',modal)){const actions=$('.hv-modal-actions',modal);actions?.insertAdjacentHTML('beforebegin','<p class="hv-day-explain">NEXT DAY를 누르면 다음 날로 넘어가고 행동 횟수가 3회로 회복돼.</p>')}}
...
document.addEventListener('click',e=>{const t=e.target instanceof Element?e.target:null;if(!t)return;if(t.closest('[data-hv-hotel-meet-continue]')){e.preventDefault();finishHotelMeeting();return}const next=t.closest('[data-hv-letter-next]');if(next){e.preventDefault();const root=$('#hvGameModal');if(root)letterDoor(root,read());return}const final=t.closest('[data-hv-letter-final]');if(final){e.preventDefault();const root=$('#hvGameModal');if(root)resolveLetter(final.dataset.hvLetterFinal,root);return}if(t.closest('[data-hv-day-guide]')){e.preventDefault();const state=read();const original=$('[data-hv-end-day]');if(original){original.click();setTimeout(schedule,0)}else if(state.game?.ended){const nextDay=$('[data-hv-next-day]');nextDay?.click()}return}},false);
