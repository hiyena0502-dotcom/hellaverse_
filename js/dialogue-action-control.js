(()=>{
'use strict';
if(window.__HELLAVERSE_DIALOGUE_ACTION_CONTROL_V3__)return;
window.__HELLAVERSE_DIALOGUE_ACTION_CONTROL_V3__=1;
...
function ensureActionButton(){
  ...
  const ask=$('[data-vn-ask]',utility),inventory=$('[data-inventory-open]',utility),leave=$('[data-vn-leave]',utility);
  ...
  if(inventory)utility.insertBefore(button,inventory);else if(leave)utility.insertBefore(button,leave);else utility.appendChild(button);
}
function openTalkPicker(){
  const room=$('.character-room');if(!room||!actionPending)return;
  const talk=$('[data-action="TALK"]',room);if(!talk)return;
  window.__HV_ACTION_PICKER_OPENING__=true;bridge=true;
  try{talk.click()}finally{bridge=false;window.__HV_ACTION_PICKER_OPENING__=false}
  setTimeout(schedule,0);
}
function beginActionPicker(){
  if(actionPending)return;
  actionPending=true;
  const box=$('.character-room .dialogue-box');if(box)clickBridge({'data-end':''});
  setTimeout(openTalkPicker,0);setTimeout(openTalkPicker,80);
}
function filterActionPicker(){
  ...
  const state=read(),byId=new Map((state.dialogues||[]).map(sc=>[String(sc.id),sc]));let count=0;
  for(const button of buttons){
    const scene=byId.get(String(button.dataset.scene||'')),visible=!!scene&&fileOf(scene,state)==='ACTION';
  ...
  if(!count){if(!empty){empty=document.createElement('p');empty.className='empty-state';empty.dataset.hvActionEmpty='1';empty.textContent='현재 사용할 수 있는 Action이 없습니다.';box.appendChild(empty)}}else empty?.remove();
  requestAnimationFrame(()=>{if(box.isConnected){box.style.removeProperty('visibility');box.style.removeProperty('pointer-events')}});
}
...
window.addEventListener('click',event=>{
  ...
  if(target.closest('[data-hv-action]')){event.preventDefault();event.stopImmediatePropagation();beginActionPicker();return}
