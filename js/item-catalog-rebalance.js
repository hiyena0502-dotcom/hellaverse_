(()=>{
'use strict';
if(window.__HELLAVERSE_ITEM_CATALOG_REBALANCE_V1__)return;
window.__HELLAVERSE_ITEM_CATALOG_REBALANCE_V1__=1;
...
function write(s){const v=JSON.stringify(s);localStorage.setItem(K,v);try{window.dispatchEvent(new StorageEvent('storage',{key:K,newValue:v}))}catch{}window.dispatchEvent(new CustomEvent('hellaverse:state-updated',{detail:{source:'item-catalog-rebalance',clearDirty:false}}))}
const OWNER=[
...
function run(){
 const s=read();s.collectionItems=Array.isArray(s.collectionItems)?s.collectionItems:[];const old=s.inventoryV2&&typeof s.inventoryV2==='object'?s.inventoryV2:{};old.version=2;old.basicItems=Array.isArray(old.basicItems)?old.basicItems:[];old.counts=old.counts&&typeof old.counts==='object'?old.counts:{};old.giftRules=old.giftRules&&typeof old.giftRules==='object'?old.giftRules:{};s.inventoryV2=old;
 let changed=false;const keep=[];
 for(const i of s.collectionItems){const name=normalize(i?.name||i?.title);if(ICONS[name]&&i.symbol!==ICONS[name]){i.symbol=ICONS[name];changed=true}const own=OWNER.find(([re])=>re.test(name));if(own&&i.characterId!==own[1]){i.characterId=own[1];changed=true}if(name==='칠죄종 공동 서명 카드'&&i.characterId!=='satan'){i.characterId='satan';changed=true}if(BASIC.has(name)){i.gachaEnabled=false;i.inventoryType='BASIC';i.collectionVisible=false;let j=old.basicItems.findIndex(x=>String(x.id)===String(i.id));if(j<0)old.basicItems.push({...i});else old.basicItems[j]={...old.basicItems[j],...i};changed=true;continue}i.inventoryType='COLLECTION';i.collectionVisible=true;keep.push(i)}
 if(keep.length!==s.collectionItems.length){s.collectionItems=keep;s.items=keep;changed=true}
 const profiles={satan:'SATAN COLLECTION',beelzebub:'BEELZEBUB COLLECTION',mammon:'MAMMON COLLECTION',asmodeus:'ASMODEUS COLLECTION',belphegor:'BELPHEGOR COLLECTION',leviathan:'LEVIATHAN COLLECTION',alastor:'ALASTOR COLLECTION','charlie-morningstar':'CHARLIE COLLECTION',niffty:'NIFFTY COLLECTION',emily:'EMILY COLLECTION',vaggie:'VAGGIE COLLECTION'};s.collectionProfiles=s.collectionProfiles||{};for(const [id,title] of Object.entries(profiles)){s.collectionProfiles[id]={...(s.collectionProfiles[id]||{}),title}}
 const paper=s.collectionItems.find(i=>normalize(i.name)==='사탄이 보낸 흑요석 인장 문진');if(paper){old.giftRules[paper.id]=old.giftRules[paper.id]||{};if(!old.giftRules[paper.id]['lucifer-morningstar'])old.giftRules[paper.id]['lucifer-morningstar']={preference:'DISLIKED',affectionDelta:0,opening:'당신은 흑요석 인장 문진을 루시퍼에게 내민다.',response:'루시퍼는 검은 문진을 보자마자 미간부터 찌푸린다. “...잠깐. 이거 사탄 거잖아.”',afterEvent:'',setFlags:'',removeFlags:'',moodChange:'ANNOYED',memoryTitle:'사탄의 문진을 루시퍼에게 전달함',memorySummary:'사탄에게 받은 흑요석 인장 문진을 루시퍼에게 건넸다.',memoryTags:'gift, satan, paperweight',approaches:[{id:'personal',label:'내가 주는 선물이야.',type:'speech',playerLine:'내가 주는 선물이야.',response:'루시퍼는 문진과 당신을 번갈아 본다. “네가? 나한테? ...취향이 꽤 무겁네. 문자 그대로.”',affectionDelta:1,afterEvent:''},{id:'relay',label:'사탄이 전해주래.',type:'speech',playerLine:'사탄이 전해주래.',response:'루시퍼의 웃는 얼굴이 아주 잠깐 굳는다. “아. 사탄. 물론이지. 굉장히... 사탄다운 전달 방식이군.”',affectionDelta:0,afterEvent:'lucifer.satan_paperweight_received'},{id:'silent',label:'말없이 건넨다.',type:'action',playerLine:'말없이 문진을 건넨다.',response:'루시퍼는 문진을 받아 들고 한참 당신을 본다. “설명은? 없어? ...좋아. 그게 더 무섭네.”',affectionDelta:-1,afterEvent:''}]};
 ...
 if(changed){s.migrations=s.migrations||{};s.migrations.itemCatalogRebalanceV1={at:new Date().toISOString()};write(s)}
}
run();
})();