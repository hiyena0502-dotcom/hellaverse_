(()=>{
'use strict';
if(window.__HELLAVERSE_RUNTIME_DIAGNOSTICS_V10__)return;
window.__HELLAVERSE_RUNTIME_DIAGNOSTICS_V10__=1;

const STATE_KEY='hellaverse_dialogue_state_v1';
const ERROR_KEY='hellaverse_runtime_errors_v1';
const INTEGRITY_KEY='hellaverse_content_integrity_report_v1';
...
function scan(){
  ...
  const integrity=parse(INTEGRITY_KEY,{})||{},integrityWarnings=Array.isArray(integrity.warnings)?integrity.warnings:[],lastFixed=Array.isArray(integrity.lastFixed)?integrity.lastFixed:[];
  return{ok:issues.length===0&&errors.length===0,issues,warnings:[...new Set(warnings)].slice(0,40),stats:{characters:chars.length,dialogues:(state.dialogues||[]).length,legacyGifts:(state.gifts||[]).length,giftAcquisitionRules:activeGiftConfigs.length,giftReactionRules,collectionItems:items.length,owned:(state.ownedItems||[]).length,rewards:(state.rewards||[]).length,contentFixes:lastFixed.length,contentWarnings:integrityWarnings.length,runtimeErrors:errors.length,runtimeErrorHits},errors,integrity:{lastFixed,lastFixedAt:String(integrity.lastFixedAt||''),warnings:integrityWarnings.slice(0,40),at:String(integrity.at||'')}};
}
function reportText(r=scan()){
  ...
  return['HELLAVERSE RUNTIME REPORT',`time: ${new Date().toLocaleString('ko-KR')}`,`status: ${r.ok?'OK':'CHECK NEEDED'}`,`characters: ${s.characters||0}`,`dialogues: ${s.dialogues||0}`,`legacyGifts: ${s.legacyGifts||0}`,
    ...
    `giftReactionRules: ${s.giftReactionRules||0}`,`collectionItems: ${s.collectionItems||0}`,`owned: ${s.owned||0}`,`rewards: ${s.rewards||0}`,`contentFixes: ${s.contentFixes||0}`,`contentWarnings: ${s.contentWarnings||0}`,`runtimeErrorTypes: ${s.runtimeErrors||0}`,`runtimeErrorHits: ${s.runtimeErrorHits||0}`,'','[ISSUES]',...(r.issues.length?r.issues:['none']),'','[WARNINGS]',...(r.warnings.length?r.warnings:['none']),'','[CONTENT INTEGRITY FIXES]',...(r.integrity?.lastFixed?.length?r.integrity.lastFixed.map(x=>`${x.type} | ${x.entity} | ${x.id} | ${x.detail}`):['none']),'','[CONTENT INTEGRITY WARNINGS]',...(r.integrity?.warnings?.length?r.integrity.warnings.map(x=>`${x.type} | ${x.entity} | ${x.id} | ${x.detail}`):['none']),'','[RECENT ERRORS]',...(r.errors.length?r.errors.slice(0,10).map(x=>`${x.time} | x${Math.max(1,Number(x.count||1))} | ${x.type} | ${x.message} | ${x.source}:${x.line||''}`):['none'])].join('\n');
}
