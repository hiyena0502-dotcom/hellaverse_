...
function conceptualQuestions(){
  ...
  for(const sc of rows){
    const mm=m?.[sc.id]||{};
    const explicit=String(mm.questionKey||mm.variantGroup||sc.questionKey||sc.variantGroup||'').trim();
    const key=explicit||`scene:${String(sc.id||'')}`;
    if(!groups.has(key))groups.set(key,[]);groups.get(key).push(sc);
  }
  ...
  return [...groups.values()].map(list=>{
    ...
    return{title:String(selected?.title||'질문').trim(),scene:selected,recommended:Number.isFinite(recommended)?recommended:0};
  }).filter(x=>x.scene).sort((a,b)=>a.recommended-b.recommended||a.title.localeCompare(b.title,'ko'));
}
...
function beginDirect(cid){direct={cid:String(cid||''),started:false,startedAt:0,deadline:performance.now()+3000};document.body.classList.add('hv-direct-dialogue-entry')}
...
function directStart(){
  ...
  if(now>direct.deadline){clearDirect();toast('대화를 시작하지 못했습니다. ENTER ROOM을 다시 눌러주세요.');return}
  const fn=window.__HV_START_CONVERSATION__;
  if(!direct.started&&typeof fn==='function'){
    direct.started=!!fn({fresh:true});
    direct.startedAt=now;
  }else if(direct.started&&now-direct.startedAt>1200&&!window.__HV_CONVERSATION_CHAIN_ACTIVE__?.()){
    // Retry only when the first start genuinely failed; never stack multiple scenes.
    direct.started=!!fn?.({fresh:true});
    direct.startedAt=now;
  }
