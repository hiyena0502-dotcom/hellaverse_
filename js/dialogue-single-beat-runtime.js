...
document.addEventListener('click',e=>{
  ...
  e.preventDefault();e.stopImmediatePropagation();const host=b.closest('.dialogue-page[data-vn-page],.dialogue-box');if(!host)return;const mode=b.dataset.singleBeatMode||'beat';
  if(mode==='beat'){host.dataset.singleBeatIndex=String(Number(host.dataset.singleBeatIndex||0)+1);host.dataset.singleBeatPhase='beat';paginate(host,$('.dialogue-page-text',host)||$('.dialogue-lines',host));return}
  if(mode==='choices'){host.dataset.singleBeatPhase='choices';paginate(host,$('.dialogue-page-text',host)||$('.dialogue-lines',host));return}
